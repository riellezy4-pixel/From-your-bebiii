...
<head>
    ...
    <style>
        ...
        body { margin: 0; font-family: 'Montserrat', sans-serif; background: var(--soft); color: #555; overflow-x: hidden; text-align: center; min-height: 200vh; }
        
        #lock { position: fixed; inset: 0; background: white; z-index: 1000; display: flex; flex-direction: column; justify-content: center; align-items: center; }
        ...
        .container { max-width: 600px; margin: 0 auto; padding: 40px 20px; position: relative; z-index: 10; }
        h1 { font-family: 'Dancing Script', cursive; font-size: 2.8rem; color: var(--dark-pink); }
        ...
        .envelope { position: relative; width: 180px; height: 120px; background: var(--peach); margin: 0 auto; border-radius: 0 0 15px 15px; transition: 0.5s; }
        ...
        .heart-seal { position: absolute; top: 40%; left: 50%; transform: translate(-50%, -50%); font-size: 40px; z-index: 3; }
        ...
        @keyframes popIn { from { transform: scale(0); opacity: 0; } to { transform: scale(1); opacity: 0.6; } }

        .floating-heart { position: fixed; pointer-events: none; z-index: 100; animation: float 4s linear infinite; }
...
<body>
    ...
    <div id="love-background" style="position: absolute; width: 100%; height: 100%; top: 0; left: 0; overflow: hidden;"></div>

    <div class="container">
        <div id="question-area" style="display:none;">
            <h1>Will you be my Valentine? 🧸</h1>
            <button class="btn" onclick="showEnvelope()">Yes! ✨</button>
        </div>

        <div id="envelope-wrapper" onclick="revealLetter()">
            <div class="envelope" id="env"><div class="heart-seal">💌</div></div>
            <p style="font-family: 'Gaegu'; font-size: 1.3rem;">(Tap the envelope)</p>
        </div>
        ...
        <div id="content">
            ...
            <h2 style="font-family: 'Dancing Script'; color: var(--dark-pink); margin-top: 50px;">Scroll down for a surprise...</h2>
        </div>
    </div>
    ...
    <script>
        ...
        function spreadLoves() {
            ...
            const interval = setInterval(() => {
                ...
                span.className = 'spread-love';
                span.textContent = `I love you ${icons[count % icons.length]}`;
                
                // Random position across the entire scrollable height
                const x = Math.random() * 90; // 0-90% width
                const y = Math.random() * 4000; // Large height to cover scrolling area
                
                span.style.left = x + 'vw';
                span.style.top = (y + 500) + 'px'; // Start below the main header
                span.style.color = count % 2 === 0 ? '#ff85a1' : '#ffafcc';
                span.style.fontSize = (Math.random() * 10 + 15) + 'px';
                
                bg.appendChild(span);
                count++;
            }, 30); // Fast pop-up speed
        }
